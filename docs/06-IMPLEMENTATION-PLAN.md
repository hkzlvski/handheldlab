...
### A.1 Database Schema Implementation (6-8h)
...
#### A.1.3 Create Triggers & Functions (2h)
...
```sql
-- Devices
INSERT INTO devices (name, slug, manufacturer, cpu, gpu, ram_gb, native_resolution, sort_order)
VALUES
('Steam Deck OLED', 'steam-deck-oled', 'Valve', 'AMD Zen 2', 'RDNA 2', 16, '800p', 1),
('ROG Ally', 'rog-ally', 'ASUS', 'AMD Ryzen Z1 Extreme', 'RDNA 3', 16, '1080p', 2),
('Legion Go', 'legion-go', 'Lenovo', 'AMD Ryzen Z1 Extreme', 'RDNA 3', 16, '1600p', 3);

-- Games
INSERT INTO games (name, slug, steam_app_id, status)
VALUES
('Elden Ring', 'elden-ring', 1245620, 'approved'),
('Cyberpunk 2077', 'cyberpunk-2077', 1091500, 'approved'),
('Hades', 'hades', 1145360, 'approved');
```
...
1. Create `app/api/reports/route.ts`:
   ...
   - Insert report into DB (status='pending')
...
const { data: reports } = await supabase
  ...
  .eq(deviceFilter ? 'device_id' : 'true', deviceId)
...
1. Create `app/api/admin/reports/[id]/reject/route.ts`:
   ...
   - Update report (status='rejected', reason, moderated_at/by)
...
**Submit Flow:**
- [ ] Submit report with all fields → pending
...
**Status:** 06-IMPLEMENTATION-PLAN.md COMPLETE ✅
